...
export class PaymentComponent implements OnInit {
  ...
  cancel() {
    this.paymentService.cancel(this.bookedConfirm).subscribe ((data: any)=> {
      this.successMessage = "Pago anulado. Revise su bandeja de entrada"
    })
  }
  confirm(){
    ...
    this.paymentService.confirm(paymentConfirm).subscribe((data: any)=> {
      this.successMessage = "Pago confirmado!. Revise su bandeja de entrada"
    })
  }
  
  buy(): void {
    const name = this.stripeTest.get("name")?.value;
    const card = this.card?.getCard(); // Verificación de nulidad con el operador '?'
  if (card) {
    this.stripeService
